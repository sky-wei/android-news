...
class NewsPagingRemoteSource(
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val result = newsService.getHeadLine(
                tid, nextPage * 10, nextPage * 20
            )
            val result1 = headLineMapper
                .transform(result)
                .also {
                    cache.saveHeadLine(
                        tid, nextPage * 10, nextPage * 20, it
                    )
                }
            LoadResult.Page(
                data = result1.lineItems,
                prevKey = if (nextPage == 1) null else nextPage - 1,
                nextKey = if (result1.lineItems.isEmpty()) null else nextPage + 1
            )
        } catch (e: Exception) {
